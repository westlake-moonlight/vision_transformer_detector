...
def transformer_preprocessor(inputs, patch_size,
    ...
    Returns:
        ...
        position_encoding: 一个 tf.float32 类型的 Keras 张量，形状为 (1, 41209)。
            需要将其输入给 keras.Model() 的 input 参数，这样才能创建出正确的模型结构图。
    """
    ...
    # patches 形状为 (batch_size, 41209, 27)，将其转为 3D 张量。
    ...
    # linear_projection 形状为 (batch_size, 41209, output_dim)，是对 patches 进行
    # 线性变换。
    ...
    # image_patches 形状为 (None, 41209, output_dim)。
...
def create_vision_transformer_detector(
    ...
    # embedded_image_patches 形状为 (batch_size, objects_quantity, 85)。
    # , position_encoding
    ...
    # encoder_outputs 形状为 (batch_size, objects_quantity, 85)。
